...
def save():
    ...
    output_wb = load_workbook('勞工報告表格範例.xlsx')
...
def check_char(data_in):
    ...
    with open('word.txt', 'r', encoding='utf_8_sig') as f:
        word = f.read().split(',')
            ...
            try:
                if date_str[0:4].isnumeric() and int(date_str[0:4]) <= date.today().year and int(date_str[0]) != 0:
                    ...
                    try:
                        ...
                        if int(date_str[0]) == 0:                        
                            newdate = datetime.strptime(str(1911 + int(date_str.split('/')[0])) + date_str[len(date_str.split('/')[0]):], '%Y/%m/%d').strftime('%Y/%m/%d')
                            return pd.Series([data_in[''] + ' (生日有誤)', newdate])
...
def add_char():
    ...
    if add == '':
        ...
        with open('word.txt', 'w', encoding='utf_8_sig') as f:
            f.write(','.join(word))
...
select_image = load_image('xls-file.png', 35)
...
refresh_image = load_image('refresh.png', 35)
...
save_image = load_image('floppy-disk.png', 35)
...
add_image = load_image('add.png', 25)
...
minus_image = load_image('minus.png', 25)
